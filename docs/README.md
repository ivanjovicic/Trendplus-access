# Docs Index

This folder collects the cutoff analysis, rollout gates, and supporting references for the Trendplus Access 2020 plan.

## Core Documents

- [ACCESS_2020_CUTOFF_ANALYSIS.md](ACCESS_2020_CUTOFF_ANALYSIS.md)
- [SALES_BY_SUPPLIER_AND_SHOE_TYPE_PLAN.md](SALES_BY_SUPPLIER_AND_SHOE_TYPE_PLAN.md)
- [ACCESS_OBJECT_CLASSIFICATION.md](ACCESS_OBJECT_CLASSIFICATION.md)
- [ACCESS_2020_TEST_MATRIX.md](ACCESS_2020_TEST_MATRIX.md)
- [ACCESS_2020_GO_NO_GO.md](ACCESS_2020_GO_NO_GO.md)
- [BACKUP_AND_ROLLBACK.md](BACKUP_AND_ROLLBACK.md)
- [SQL_IMPORT_PREP.md](SQL_IMPORT_PREP.md)

## SQL References

- [sql/README.md](../sql/README.md)
- [sql/reporting-2020/README.md](../sql/reporting-2020/README.md)
- [sql/audit/README.md](../sql/audit/README.md)
